...
class naiveBayes {
	...
	public function __construct ($trainingSet = array(), $type=NULL) {
		...
		if ($this->debug) 
			echo "Debug Info >> NaiveBayes: type -> $type\n";
		if ($type == 'numeric')
...
class nbNumeric {
	...
	public function train () {
		...
		foreach ($priorNums as $class => $pn) {
			...
			if ($this->debug) 
				echo "Debug Info >> NaiveBayes: Prior Probability [$class] = {$this->priorProbs[$class]} ($pn/$trainDocNum)\n";
		}
		...
		foreach ($this->fl as $feature=>$_) {
			foreach ($this->priorProbs as $class=>$_) {// Laplace estimate
				...
				if ($this->debug) 
						echo "Debug Info >> NaiveBayes: Feature Probability [$feature.$class] = {$this->featureProbs[$feature][$class]} " .
	...
	public function prob (&$data, $class) {
		...
		foreach ($this->priorProbs as $c=>$_) {
			...
			if ($this->debug)
				echo "Debug Info >> Naive Bayes: Unnormalized testing probability [$c] = ".$this->probUnn($data, $c).", (logP = $p) \n";
			if ($c==$class) 
				$prob = exp($p);
			$fullProb += exp($p);
		}
			...
			foreach ($this->priorProbs as $class=>$_) {
				...
				if ($zeroflag) {
					foreach ($fvalue_ as $fvalue => $frq) { // Laplace
						...
						if ($this->debug) 
							echo "Debug Info >> NaiveBayes: Feature Probability [$fname.$fvalue.$class] = {$this->featureProbs[$fname][$class][$fvalue]} " .
