...
class obo {
	...
	public function &root() {
		...
		foreach ($this->obo['Term'] as $id => $term) {
			...
			if (sizeof($term->is_a())==0 and sizeof($term->part_of())==0 and !$term->is_obsolete()) {
				$roots[] = $term;
			}
		}
	...
	public function &tree (&$rtree = NULL) {
		...
		foreach ($this->obo['Term'] as $term) {
			$id = $term->id();
			foreach (array_merge((array)$term->part_of(), (array)$term->is_a()) as $father) {
				$tree[$father][] = $id;
	 ...
	 * @return: array(id1, ...)
	 */
	public function &children($id, $all=false) {
		$ret = array();
		if (!$all) {
			foreach ($this->obo['Term'] as $term) {
				if (in_array ($id, array_merge((array)$term->part_of(), (array)$term->is_a())))
					$ret[] = $term->id();
			}
		} else {
			$tree = $this->tree();
			...
			while (sizeof($children)) {
				...
				foreach ($children1 as $child) {
					...
					$children = array_merge($children, (array)$tree[$child]);
				}
			}
		}
		return $ret;
...
class oboterm {
	...
	/**
	 * @desc: get the term ids that have the relation part_of with this term
	 * @return: array(id1,...)
	 */
	public function part_of() {
		if (!isset($this->fields['relationship'])) return array();
		$ret = array();
		foreach ((array)$this->fields['relationship'] as $relationship) {
			if (strpos($relationship, 'part_of ')!==0) continue;
			$part_of = substr($relationship, 8);
			$tmp = explode(' ! ', $part_of);
			$ret[] = $tmp[0];
		}
		return $ret;
